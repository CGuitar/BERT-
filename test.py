...
if __name__ == "__main__":
    ...
    # 输入CSV文件路径
    input_csv_path = "unique_reviews.csv"  # 替换为你的输入CSV文件路径
    output_csv_path = "output.csv"  # 替换为你的输出CSV文件路径

    # 读取CSV文件
    with open(input_csv_path, mode='r', encoding='utf-8') as infile:
        reader = csv.DictReader(infile)
        fieldnames = reader.fieldnames + ["label", "scores"]  # 添加新的列名
        rows = list(reader)  # 将所有行读取到内存中

    # 对每一行进行预测
    for row in rows:
        text = row.get("评价内容", "")  # 假设CSV文件中有一列名为"评价内容"
        if text:
            result = json.loads(parse_handler.predict(text))[0]  # 获取预测结果
            row["label"] = result["label"]  # 添加预测标签
            row["scores"] = result["scores"]  # 添加预测分数
