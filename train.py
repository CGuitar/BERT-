...
class MyPro(DataProcessor):
    """
    自定义数据读取方法，针对json文件
    Returns: examples: 数据集，包含index、中文文本、类别三个部分
    """
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, show_exp=True):
    ...
    :param examples: [List] 输入样本，包括question, label, index
    :param label_list: [List] 所有可能的类别，可以是int、str等，如['book', 'city', ...]
    :param max_seq_length: [int] 文本最大长度
    :param tokenizer: [Method] 分词方法
    :param show_exp:
    :return: features:
...
def main():
    ...
    # 学习率调度器
    total_steps = len(train_examples) * args.num_train_epochs
    optimizer = AdamW(optimizer_grouped_parameters, lr=args.learning_rate, correct_bias=False)
    scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=total_steps * args.warmup_proportion,
